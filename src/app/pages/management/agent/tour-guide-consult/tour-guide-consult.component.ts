...
import * as tourGuideActions from 'src/app/store/actions/tour-guide.actions';
import { KeyValueTypePerson, TypePersonEnum, Agent, EventFilters, Options, User } from 'src/app/models';

@Component({
  selector: 'app-tour-guide-consult',
  templateUrl: './tour-guide-consult.component.html',
  styles: [],
  standalone: true,
  imports: [TableModule, SharedModule, MultiSelectModule, TooltipModule, MaskDocumentPipe, TitleCasePipe],
})
export class TourGuideConsultComponent extends BaseResourceListComponent<Agent.Model> implements OnInit {
  ...
  public activateHandler(payload: number) {
    this.store.dispatch(tourGuideActions.activationTourGuide({ payload }));
  }

  public inactivateHandler(payload: number) {
    this.store.dispatch(tourGuideActions.deactivateTourGuide({ payload }));
  }
  ...
  protected toDataPdf() {
    ...
    for (let i = 0; i < this.resources.length; i++) {
      this.dataPdf.push([
        ...
        this.setDescription.status(this.resources[i].statusTourGuide),
