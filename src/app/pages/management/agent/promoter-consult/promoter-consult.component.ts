...
import * as promoterActions from 'src/app/store/actions/promoters.actions';
import * as usersActions from 'src/app/store/actions/users.actions';
import { MaskDocumentPipe } from '../../../../shared/mask-document.pipe';

@Component({
  selector: 'app-promoter-consult',
  templateUrl: './promoter-consult.component.html',
  styles: [],
  standalone: true,
  imports: [TableModule, SharedModule, MultiSelectModule, TooltipModule, MaskDocumentPipe, TitleCasePipe],
})
export class PromoterComponent extends BaseResourceListComponent<Agent.Model> implements OnInit {
  ...
  public activateHandler(payload: number) {
    this.store.dispatch(promoterActions.activationPromoter({ payload }));
  }

  public inactivateHandler(payload: number) {
    this.store.dispatch(promoterActions.deactivatePromoter({ payload }));
  }
  ...
  protected toDataPdf() {
    ...
    for (let i = 0; i < this.resources.length; i++) {
      this.dataPdf.push([
        ...
        this.setDescription.status(this.resources[i].statusPromoter),
