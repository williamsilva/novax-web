...
import * as providerActions from 'src/app/store/actions/providers.actions';
...
export class ProviderComponent extends BaseResourceListComponent<Agent.Model> implements OnInit {
  ...
  public activateHandler(payload: number) {
    this.store.dispatch(providerActions.activationProvider({ payload }));
  }

  public inactivateHandler(payload: number) {
    this.store.dispatch(providerActions.deactivateProvider({ payload }));
  }
  ...
  protected toDataPdf() {
    ...
    for (let i = 0; i < this.resources.length; i++) {
      this.dataPdf.push([
        ...
        this.setDescription.status(this.resources[i].statusProvider),
